...
const createQueryObject = ({ key, value, bitmap }) => ({
	key: Buffer.from(key, 'hex'),
	value: Buffer.from(value, 'hex'),
	bitmap: Buffer.from(bitmap, 'hex'),
	binaryBitmap: utils_1.bufferToBinaryString(Buffer.from(bitmap, 'hex')),
});
describe('utils', () => {
	describe('sortByBitmapAndKey', () => {
		it('should sort by longest bitmap', () => {
			const res1 = {
				key: lisk_cryptography_1.getRandomBytes(2),
				binaryBitmap: '011',
			};
			const res2 = {
				key: lisk_cryptography_1.getRandomBytes(2),
			...
			expect(utils_1.sortByBitmapAndKey([res1, res2])).toEqual([res2, res1]);
		});
		it('should sort by longest bitmap breaking tie with smaller key', () => {
			...
			const res2 = {
				key: lisk_cryptography_1.getRandomBytes(1),
				binaryBitmap: '111',
			};
			expect(utils_1.sortByBitmapAndKey([res1, res2])).toEqual([res2, res1]);
		});
	...
	describe('filterQueries', () => {
		it('should remove queries which are merged together', () => {
			...
			expect(utils_1.filterQueries([q1, q2, q3, q4], 1)).toEqual([q1, q2, q4]);
		});
	});
	describe('calculateRoot', () => {
		it('should calculate correct root hash for single proof', () => {
			...
			expect(utils_1.calculateRoot(siblingHashes, queries, 1).toString('hex')).toEqual(rootHash);
		});
	...
	describe('binaryStringToBuffer', () => {
		it.each(binarySampleData)('should convert binary string "%o" to correct buffer value', data => {
			expect(utils_1.binaryStringToBuffer(data.str)).toEqual(Buffer.from(data.buf, 'hex'));
		});
	});
	describe('bufferToBinaryString', () => {
		it.each(binarySampleData)('should convert buffer "%o" to correct binary string', data => {
			expect(utils_1.bufferToBinaryString(Buffer.from(data.buf, 'hex'))).toEqual(data.str);
		});
	});
	describe('getOverlappingStr', () => {
		it('returns valid overlapping strings', () => {
			expect(utils_1.getOverlappingStr('12356', '123456')).toEqual('123');
			expect(utils_1.getOverlappingStr('1010110', '1010010')).toEqual('1010');
		});
		it('returns empty string if no overlapping strings found', () => {
			expect(utils_1.getOverlappingStr('12356', '2356')).toEqual('');
			expect(utils_1.getOverlappingStr('0110011', '110011')).toEqual('');
		});
	});
	describe('verify', () => {
		it('should return false if number of query keys are not same as proof', () => {
			const queryKeys = ['01111110', '01101101', '00011011'].map(utils_1.binaryStringToBuffer);
			const proof = {
				siblingHashes: [
					'e6fa536eaac055d524e29fb4682893e3111bf3a027f7cd5ba312aec56460eb1b',
					'63a154f88e6f5898bada58cbcb0dfcfa84e18cd5d50783e6703d904bef8be36b',
			...
			expect(utils_1.verify(queryKeys, proof, merkleRoot, 1)).toBeFalse();
		});
		it('should return false if queries does not match and proof does not provide inclusion proof of one of query key', () => {
			const queryKeys = ['00110011', '01100000'].map(utils_1.binaryStringToBuffer);
			const proof = {
				siblingHashes: [
		...
		it('should return true if queries do not match but response query object provides a matching/valid merkle root', () => {
			const queryKeys = ['01011010', '10101000'].map(utils_1.binaryStringToBuffer);
			const proof = {
				siblingHashes: [
			...
			expect(utils_1.verify(queryKeys, proof, merkleRoot, 1)).toBeTrue();
		});
		it('should return true for matching queries and root', () => {
			const queryKeys = ['01011010', '10101001'].map(utils_1.binaryStringToBuffer);
			const proof = {
				siblingHashes: [
					'e041e1c0e364cc015af04118c1cd5a6554a7b357727ed937aac49436f0fbbf9c',
					'6400721efe3b54db24f248b0d8c93c0aa21eee1eebca058a143da44abeeea0e3',
	...
	describe('binaryExpansion', () => {
		...
		for (const data of sampleData) {
			it(`should return correct binary string for ${data.keyLength} byte size`, () =>
				expect(data.str).toEqual(utils_1.binaryExpansion(data.buf, data.keyLength)));
		}
	});
	describe('isLeaf', () => {
		...
		const leafDataBuffer = utils_1.leafData(sampleKey, sampleValue);
		const branchDataBuffer = utils_1.branchData(sampleKey, sampleValue);
		it('Should return true when a leaf node data is passed', () =>
			expect(utils_1.isLeaf(leafDataBuffer)).toEqual(true));
		it('Should return false when a branch node data is passed', () =>
			expect(utils_1.isLeaf(branchDataBuffer)).toEqual(false));
	});
	describe('parseLeaf', () => {
		const sampleKey = Buffer.from('46', 'hex');
		const sampleValue = Buffer.from(
		...
		it('Should return key value from leaf data buffer', () =>
			expect(utils_1.parseLeafData(leafDataBuffer, 1)).toEqual({
				key: sampleKey,
				value: sampleValue,
			}));
		it('should get key value of 38 bytes from leaf data buffer', () => {
			const key38Bytes = lisk_cryptography_1.getRandomBytes(38);
			const leafDataWith38ByteKey = utils_1.leafData(key38Bytes, sampleValue);
			expect(utils_1.parseLeafData(leafDataWith38ByteKey, key38Bytes.byteLength)).toEqual({
				key: key38Bytes,
				value: sampleValue,
			});
		});
	...
	describe('parseBranch', () => {
		...
		const branchDataBuffer = utils_1.branchData(leftHash, rightHash);
		it('Should return key value from leaf data buffer', () =>
			expect(utils_1.parseBranchData(branchDataBuffer)).toEqual({
				leftHash,
				rightHash,
			}));
	});
