...
export class FastSkipMerkleTree {
	public counter = 0;
	...
	private async _updateSubtree(
        ...
        for (let i = 0; i < currentSubtree.nodes.length; i+=1) {
            ...
            const newV = V + (1 << (this.subtreeHeight - h));
            const sliceKeys = binKeys.slice(V, newV);
            const lengthBase = cumulativeSum(sliceKeys.map(kb => kb.length));

            const [_nodes, _heights] = await this._updateNode(
                sliceKeys, binValues.slice(V, newV), lengthBase, 0, currentNode, height, h
            )
    ...
    private async _updateNode(
        ...
        const totalData = lengthBins[lengthBins.length-1] - lengthBase;
        ...
        if (totalData === 1) {
            const idx = lengthBins.findIndex(el => el === lengthBase + 1);
			...
			if (currentNode instanceof Leaf) {
				if (currentNode.key.equals(keyBins[idx][0])) {
                    const newLeaf = Leaf.fromData(keyBins[idx][0], valueBins[idx][0]);
                    return [[newLeaf], [h]];
                }
        ...
        const [leftNodes, leftHeights] = await this._updateNode(
            keyBins.slice(0, idx), valueBins.slice(0, idx), lengthBins.slice(0, idx), lengthBase, leftNode, height, h + 1
        );
        const [rightNodes, rightHeights] = await this._updateNode(
            keyBins.slice(idx), valueBins.slice(idx), lengthBins.slice(idx), lengthBins[idx-1], rightNode, height, h + 1
        );
